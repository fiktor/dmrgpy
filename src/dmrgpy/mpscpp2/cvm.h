...
#ifndef __MPSCPP2_CVM_H
#define __MPSCPP2_CVM_H
...
MPS bicstab(MPO A, MPS b, double tol, int max_it, Args const &args) {
  ...
  while (k < max_it) {
    ...
    beta = (alpha / w) * overlapC(conjMPS(r_new), r_) /
           overlapC(conjMPS(r_old), r_);
...
// main CVM function
static auto spectral_function =
    [](MPS psi, MPO H, MPO S1, MPO S2, double omega, double eta, double energy,
       double tol, int max_it, int maxm, double cut, auto sites) {
