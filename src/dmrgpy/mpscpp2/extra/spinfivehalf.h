...
public:
  ...
  SpinFiveHalf(int N, Args const &args = Args::global());
...
public:
  ...
  SpinFiveHalfSite(int n, Args const &args = Args::global()) {
    s = IQIndex{nameint("S=2 site=", n),
                Index(nameint("Up:site", n), 1, Site),
                QN("Sz=", +5),
                Index(nameint("Upi:site", n), 1, Site),
                QN("Sz=", +3),
                Index(nameint("Upii:site", n), 1, Site),
                QN("Sz=", +1),
                Index(nameint("Dnii:site", n), 1, Site),
                QN("Sz=", -1),
                Index(nameint("Dni:site", n), 1, Site),
                QN("Sz=", -3),
                Index(nameint("Dn:site", n), 1, Site),
                QN("Sz=", -5)};
  }
...
inline SpinFiveHalf::SpinFiveHalf(int N, Args const &args) {
...
void inline SpinFiveHalf::read(std::istream &s) {
  ...
  if (N > 0) {
    ...
    for (int j = 1; j <= N; ++j) {
      ...
      if (I.m() == 3)
        ...
        Error(format("SpinFiveHalf cannot read index of size %d", I.m()));
    }
    init(std::move(store));
  }
}
...
#endif // __ITENSOR_SPINFIVEHALF_H
