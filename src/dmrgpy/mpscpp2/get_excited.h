...
#ifndef __MPSCPP2_GET_EXCITED_H
#define __MPSCPP2_GET_EXCITED_H

#include "get_gs.h"
#include "get_hamiltonian.h"
#include "get_sites.h"
#include "get_sweeps.h"
#include "mpsalgebra.h"

float get_energy_fluctuation(auto psi1, auto H) {
  normalize(psi1); // normalize the wavefunction
  auto psi2 = exactApplyMPO(
      psi1, H,
      {"Maxm", get_int_value("maxm"), "Cutoff", get_float_value("cutoff")});
  float de;
...
static auto get_excited = []() {
  ...
  // lagrange multiplier
  float weight =
      bandwidth(sites, H) * get_float_value("scale_lagrange_excited");
  ...
  if (get_bool("excited_gram_schmidt"))
    wfs = gram_schmidt(wfs);
