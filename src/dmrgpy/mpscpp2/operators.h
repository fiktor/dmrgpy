...
#ifndef __MPSCPP2_OPERATORS_H
#define __MPSCPP2_OPERATORS_H
...
static auto get_operator = [](auto sites, int i, auto name) {
  if (name == "Id")
    return Iden(sites); // return identity
  auto ampo = AutoMPO(sites);
...
static auto add_spin_operator = [](auto ampo, auto sites, float v, int i,
                                   auto name) {
  if (site_type(i) != 1) // spin site
    ampo += v, name, i + 1;
