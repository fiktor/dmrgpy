...
#ifndef __MPSCPP2_CVM_DYNAMICAL_CORRELATOR_H
#define __MPSCPP2_CVM_DYNAMICAL_CORRELATOR_H

#include "get_ampo_operator.h"
#include "get_gs.h"
...
static auto cvm_dynamical_correlator = []() {
  ...
  auto z = spectral_function(psi, H, A1, A2, omega, delta, e0, tol, max_it,
                             maxm, cutoff, sites); // return the correlator
  // end of the call
  ofstream filecvm;
  filecvm.open("CVM.OUT"); // open file
  filecvm << std::setprecision(8) << real(z) << "  " << std::setprecision(8)
          << imag(z) << endl;
  filecvm.close(); // close file
};
#endif // __MPSCPP2_CVM_DYNAMICAL_CORRELATOR_H
