...
#ifndef __MPSCPP2_KPMCORRELATOR_H
#define __MPSCPP2_KPMCORRELATOR_H
...
static auto moments_vi_vj_shift = [](auto m, auto vi, auto vj, int n,
                                     auto shift) {
  ofstream myfile;
  ...
  for (i = 0; i < n; i++) {
    ...
    ap = sum(ap, -1.0 * am,
             {"Maxm", kpmmaxm, "Cutoff", 1E-7}); // recursion relation
...
static auto get_moments_spismj_brute = [](auto sites, auto H, int n, int i,
                                          int j) {
...
#endif // __MPSCPP2_KPMCORRELATOR_H
