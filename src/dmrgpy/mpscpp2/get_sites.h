...
#ifndef __MPSCPP2_GET_SITES_H
#define __MPSCPP2_GET_SITES_H
...
void inline SpinX::read(std::istream &s) {
  ...
  if (N > 0) {
    ...
    for (int j = 1; j <= N; ++j) {
      ...
      if (nm == 3)
        store.set(j, SpinOneSite(I));
      else if (nm == 0)
        store.set(j, SpinlessSite(I)); // use fermions
      else if (nm == 1)
        store.set(j, HubbardSite(I));
      else if (nm == 2)
        store.set(j, SpinHalfSite(I));
      else if (nm == 4)
        store.set(j, SpinThreeHalfSite(I));
      else if (nm == 5)
        store.set(j, SpinTwoSite(I));
      else if (nm == 6)
        store.set(j, SpinFiveHalfSite(I));
      else if (nm == -2)
        store.set(j, Z3Site(I));
      else if (nm == -3)
        store.set(j, Z4Site(I));
      else if (nm == 104)
        store.set(j, BosonFourSite(I));
      else
        Error(format("SpinX cannot read index of size %d", nm));
    }
    sfile.close();
    init(std::move(store));
  }
...
inline SpinX::SpinX(Args const &args) {
  ifstream sfile;         // file to read
  sfile.open("sites.in"); // file with the coupling
  int N, nm;
  ...
  for (int i = 1; i <= N; i++) {
    ...
    if (nm == 2)
      sites.set(i, SpinHalfSite(i)); // use spin=1/2
    else if (nm == 0)
      sites.set(i, SpinlessSite(i)); // use fermions
    else if (nm == 1)
      sites.set(i, HubbardSite(i)); // use spinful fermions
    else if (nm == 3)
      sites.set(i, SpinOneSite(i)); // use spin=1
    else if (nm == 4)
      sites.set(i, SpinThreeHalfSite(i)); // use spin=3/2
    else if (nm == 5)
      sites.set(i, SpinTwoSite(i)); // use spin=2
    else if (nm == 6)
      sites.set(i, SpinFiveHalfSite(i)); // use spin=5/2
    else if (nm == 104)
      sites.set(i, BosonFourSite(i)); // use Boson
    else if (nm == (-2))
      sites.set(i, Z3Site(i)); // use Z3
    else if (nm == (-3))
      sites.set(i, Z4Site(i)); // use Z4
    else
      Error(format("SpinX cannot read index of size "));
  };
...
auto get_sites() {               // function to get the sites
  ...
  // overwrite the sites
  if (check_task("gs_from_file"))
    readFromFile("sites.sites", sites);
  if (check_task("sites_from_file"))
    readFromFile("sites.sites", sites);
